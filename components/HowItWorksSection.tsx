...
const StepCard: React.FC<StepCardProps> = ({ icon, title, description, stepNumber, delay }) => {
    return (
        ...
        >
            <div className="w-28 h-28 flex items-center justify-center rounded-full bg-slate-800 border-2 border-slate-700 mb-6 relative z-10 transition-all duration-300 group-hover:border-[color:var(--color-primary)]">
                <span className="absolute -top-3 -right-3 w-12 h-12 flex items-center justify-center bg-[color:var(--color-primary)] text-slate-900 font-bold rounded-full text-xl border-4 border-slate-950">
                    {stepNumber}
                </span>
